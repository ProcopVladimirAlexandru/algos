...
#include "Knapsack01Tester.h"
#include "PVAFile.h"
#include "VectorOperations.h"


void Knapsack01Tester::test(std::string input_file_path) {
    ...
    while (!pva_file.eof()) {
        if (i % rows_per_test  == 0) {
            ...
            Knapsack01 ks01("Knapsack01");
            std::vector<long double> result_quantities = ks01.get_quantities(W, weights, values);
            if (!are_vectors_equal(result_quantities, expected_quantities)) {
                std::cerr << ks01.uid << " failed test" << std::endl;
                print_vector(result_quantities, true);
            }
            else {
                std::cout << ks01.uid << " passed test" << std::endl;
            }
            std::cout << std::endl;
        }
        ++i;
